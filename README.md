...
You can check the latest commits to see the code that needed to be changed. Also, refer to our [patched version](https://github.com/jamiesyme/expo-apple-utils) of the `@expo/apple-utils` package.
...
NOTE: if the version hasn't changed since the previous build, you'll need to delete the release [here](https://github.com/jamiesyme/expo-cli/releases) before publishing again.

Then you just need to update the app builder's `expo-cli` dependency to use the new git tag generated by `gitpkg` (if different).
