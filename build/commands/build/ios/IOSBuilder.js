...
function _httpsProxyAgent() {
  const data = _interopRequireDefault(require("https-proxy-agent"));
...
class IOSBuilder extends _BaseBuilder().default {
  async run() {
    ...
    if (this.options.type === 'archive') {
      ...
      //
      const withHttpsProxy = async (url, cb) => {
        const ogAgent = _https().default.globalAgent;

        try {
          _https().default.globalAgent = new (_httpsProxyAgent().default)(url);
          return await cb();
        } finally {
          _https().default.globalAgent = ogAgent;
        }
      };
  ...
  async prepareCredentials() {
    ...
    await context.ensureAppleCtx(); //await this.bestEffortAppleCtx(context);
